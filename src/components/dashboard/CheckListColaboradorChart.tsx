...
interface CheckListColaboradorChartProps {
  filteredData: CheckListData[];
}

const CheckListColaboradorChart = ({ filteredData }: CheckListColaboradorChartProps) => {
  ...
  const chartData = React.useMemo(() => {
    const dataToUse = activeFilter.type === 'colaborador' ? filteredData : interactiveFilteredData;
    
    const colaboradorCounts = dataToUse.reduce((acc, item) => {
      const colabName = item.colaborador?.trim();
      if (colabName) {
        if (!acc[colabName]) {
          acc[colabName] = { conforme: 0, naoConforme: 0 };
        }
        
        if (Number(item.V) === 1) {
          acc[colabName].conforme++;
        } else {
          acc[colabName].naoConforme++;
        }
      }
      return acc;
    }, {} as Record<string, { conforme: number; naoConforme: number }>);

    return Object.entries(colaboradorCounts)
      .map(([colaborador, counts]) => ({
        name: colaborador,
  ...
  const handleBarClick = (data: any) => {
    if (activeFilter.type === 'colaborador' && activeFilter.value === data.name) {
      clearActiveFilter();
    } else {
      setActiveFilter({ type: 'colaborador', value: data.name });
    }
  };
        ...
        <CardTitle className="flex items-center justify-between">
          <div className="flex items-center gap-3 group-hover:text-primary transition-colors duration-300">
            <div className="p-2.5 rounded-full bg-gradient-to-br from-primary/10 to-primary/20 group-hover:from-primary/20 group-hover:to-primary/30 transition-all duration-300 group-hover:scale-110 shadow-lg relative overflow-hidden">
              ...
              <UserCheck className="h-5 w-5 text-primary relative z-10" />
            </div>
            <span className="bg-gradient-to-r from-foreground to-foreground/80 bg-clip-text">
              Inspeção por Colaborador
            </span>
          </div>
          {activeFilter.type === 'colaborador' && (
            <button
        ...
        <div className="overflow-x-auto">
          <div className="relative">
            ...
            <ChartContainer config={chartConfig} className="w-full h-[320px]">
              <ResponsiveContainer width={Math.max(1200, chartData.length * 50)} height={320}>
                <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 80 }}>
                  <defs>
                    <linearGradient id="conformeGradientColaborador" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#10b981" stopOpacity={0.8} />
                      <stop offset="100%" stopColor="#10b981" stopOpacity={0.3} />
                    </linearGradient>
                    <linearGradient id="naoConformeGradientColaborador" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Bar 
                    ...
                    fill="url(#conformeGradientColaborador)"
                    radius={[0, 0, 0, 0]}
                  ...
                  <Bar 
                    dataKey="naoConforme" 
                    stackId="conformidade"
                    fill="url(#naoConformeGradientColaborador)"
                    radius={[8, 8, 0, 0]}
                    onClick={handleBarClick}
                    className="cursor-pointer"
                  >
...
export default CheckListColaboradorChart;
