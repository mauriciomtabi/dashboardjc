...
const ManutencaoServicosChart = ({ filteredData }: ManutencaoServicosChartProps) => {
  ...
  const chartData = React.useMemo(() => {
    const servicoCounts = filteredData.reduce((acc, item) => {
      if (item.AK) {
        if (!acc[item.AK]) {
          acc[item.AK] = { preventiva: 0, corretiva: 0, custoTotal: 0 };
        }
        
        const valor = parseFloat(item.Q) || 0;
        acc[item.AK].custoTotal += valor;
      ...
      <CardHeader className="pb-2 relative z-10">
        <CardTitle className="flex items-center gap-3 group-hover:text-primary transition-colors duration-300">
          ...
          <span className="bg-gradient-to-r from-foreground to-foreground/80 bg-clip-text">
            Top 15 Serviços - Custo de Manutenção
          </span>
        </CardTitle>
      </CardHeader>
